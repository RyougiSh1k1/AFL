...
def validate_task(val_loader, model, global_model, task_classes, args):
    """
    Validate on a specific task (subset of classes)
    """
    num_correct = 0
    num_sample = 0
    model.eval()
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            # Filter samples that belong to the task classes
            mask = torch.zeros(target.size(0), dtype=torch.bool)
            ...
            images = images[mask].cuda(args.gpu, non_blocking=True)
            target = target[mask].cuda(args.gpu, non_blocking=True)
...
def calculate_forgetting(task_accuracies):
    """
    Calculate average forgetting across all tasks
    
    Args:
        task_accuracies: Dict of format {client_id: {task_id: [acc_after_task_0, acc_after_task_1, ...]}}
    
    Returns:
        avg_forgetting: Average forgetting across all tasks and clients
        forgetting_per_client: Dict with forgetting for each client
    """
    forgetting_per_client = {}
    all_forgetting = []
    
    for client_id, client_tasks in task_accuracies.items():
        client_forgetting = []
        
        for task_id, acc_history in client_tasks.items():
            if len(acc_history) > 1:
                # Maximum accuracy achieved for this task
                ...
                all_forgetting.append(task_forgetting)
        
        if client_forgetting:
            forgetting_per_client[client_id] = np.mean(client_forgetting)
    
    avg_forgetting = np.mean(all_forgetting) if all_forgetting else 0
    
    return avg_forgetting, forgetting_per_client